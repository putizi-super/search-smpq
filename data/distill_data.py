...
def getRandomData(dataset='cifar10', batch_size=512, for_inception=False):
    ...
    data_loader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=32)
    return data_loader
...
def getDistilData(teacher_model, dataset, batch_size, num_batch=1, for_inception=False):
    """
    ...
    dataloader = getRandomData(dataset=dataset, batch_size=batch_size, for_inception=for_inception)
    ...
    for n, m in teacher_model.named_modules():
        ...
        if isinstance(m, nn.BatchNorm2d):
            ...
            bn_stats.append( (m.running_mean.detach().clone().flatten().cuda(),  torch.sqrt(m.running_var + eps).detach().clone().flatten().cuda()))
