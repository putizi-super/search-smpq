...
class HybridTrainPipe(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop, 
                 local_rank=0, world_size=1, dali_cpu=False):
...
class HybridValPipe(Pipeline):
    def __init__(self, batch_size, num_threads, device_id, data_dir, crop,
                 size, local_rank=0, world_size=1, dali_cpu=False):
...
def get_imagenet_iter_dali(type, image_dir, batch_size, num_threads, device_id, crop, val_size=256,
    ...
    if type == 'train':
        pip_train = HybridTrainPipe(batch_size=batch_size, num_threads=num_threads, device_id=device_id,
                                    data_dir=image_dir + '/train', crop=crop, local_rank=local_rank,
                                    world_size=world_size, dali_cpu=dali_cpu)
        pip_train.build()
        ...
        pip_val = HybridValPipe(batch_size=batch_size, num_threads=num_threads, device_id=device_id,
                                data_dir=image_dir + '/val', crop=crop, size=val_size, local_rank=local_rank,
                                world_size=world_size, dali_cpu=dali_cpu)
        pip_val.build()
...
if __name__ == '__main__':
    train_loader = get_imagenet_iter_dali(type='train', image_dir='/userhome/memory_data/imagenet', batch_size=256,
                                          num_threads=4, device_id=0, crop=224, num_gpus=1)
    print('start iterate')
    start = time.time()
