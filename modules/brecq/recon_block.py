...
from .brecq_block import BaseQuantBlock
...
def block_reconstruction(model: QuantModel, block: BaseQuantBlock, pruning_rate: list, cali_data: torch.Tensor,
                         batch_size: int = 16, iters: int = 20000, weight: float = 0.01, 
                         opt_mode: str = 'mse', asym: bool = False, include_act_func: bool = True,
                         b_range: tuple = (10, 2), warmup: float = 0.0, act_quant: bool = False, lr: float = 4e-5):
    """
    ...
    if not act_quant:
        ...
        for name, module in block.named_modules():
            if isinstance(module, QuantModule):
                module.weight_quantizer = AdaRoundQuantizer(quantizer=module.weight_quantizer, pruning_rate = pruning_rate[i], round_mode=round_mode,
        ...
        elif hasattr(block.act_quantizer, 's') and block.act_quantizer.s is not None:
            opt_params = [block.act_quantizer.s]
        elif hasattr(block.act_quantizer, 's_max') and block.act_quantizer.s_max is not None:
            opt_params = [block.act_quantizer.s_max]
        else:
            opt_params = []
        # opt_params = []
        for name, module in block.named_modules():
            if isinstance(module, QuantModule):
                if isinstance(module.in_act_quantizer, UniformAffineQuantizer):
                    if module.in_act_quantizer.delta is not None:
                        opt_params += [module.in_act_quantizer.delta]
                elif isinstance(module.in_act_quantizer, RistrettoQuantizer):
                    if module.in_act_quantizer.s_max is not None:
                        opt_params += [module.in_act_quantizer.s_max]
                elif isinstance(module.in_act_quantizer, LSQQuantizer):
                    if module.in_act_quantizer.s is not None:
                        opt_params += [module.in_act_quantizer.s]
                if isinstance(module.out_act_quantizer, UniformAffineQuantizer):
                    if module.out_act_quantizer.delta is not None:
                        opt_params += [module.out_act_quantizer.delta]
                elif isinstance(module.out_act_quantizer, RistrettoQuantizer):
                    if module.out_act_quantizer.s_max is not None:
                        opt_params += [module.out_act_quantizer.s_max]
                elif isinstance(module.out_act_quantizer, LSQQuantizer):
                    if module.out_act_quantizer.s is not None:
                        opt_params += [module.out_act_quantizer.s]
    ...
    loss_func = LossFunction(block, round_loss=loss_mode, round_mode=round_mode, weight=weight, max_count=iters, rec_loss=rec_loss,
            ...
            for name, module in self.block.named_modules():
                if isinstance(module, QuantModule):
                    if self.round_mode == 'learned_hard_sigmoid':
                        ...
                        round_vals = module.weight_quantizer.get_tanh_soft_targets()
