...
from .brecq_layer import QuantModule, Union
from .brecq_model import QuantModel


def save_inp_oup_data(model: QuantModel, layer: QuantModule, cali_data: torch.Tensor,
                      asym: bool = False, act_quant: bool = False, weight_quant: bool = False, batch_size: int = 32, keep_gpu: bool = True):
    """
    ...
    get_inp_out = GetLayerInpOut(model, layer, device=device, asym=asym, act_quant=act_quant, weight_quant=weight_quant)
    cached_batches = []
...
def save_grad_data(model: QuantModel, layer: QuantModule, cali_data: torch.Tensor,
...
class GetLayerInpOut:
    ...
    def __call__(self, model_input):
        ...
        with torch.no_grad():
            ...
            # else:
            #     self.data_saver.store_output = False
            #     self.layer.set_quant_state(weight_quant=True, act_quant=self.act_quant)
            #     module_list = []
            #     i = 2
            ...
            #             module_list += [m]
            #             if m.use_act_quant:
            #                 if len(module_list) > i:
            #                     module_list[-2].set_quant_state(weight_quant=True, act_quant=self.act_quant)
            #                     module_list[-3].set_quant_state(weight_quant=True, act_quant=self.act_quant)
            #                     # module_list[-4].set_quant_state(weight_quant=True, act_quant=self.act_quant)
            #                 else:
            #                     for j in range(len(module_list)):
            #                         module_list[j].set_quant_state(weight_quant=True, act_quant=self.act_quant)
            #     try:
            #         _ = self.model(model_input.to(self.device))
            #     except StopForwardException:
            #         pass
