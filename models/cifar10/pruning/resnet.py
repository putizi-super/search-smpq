...
def adapt_channel(compress_rate, num_layers):
    ...
    stage_oup_cprate +=[1.0] * stage_repeat[-1]
    ...
    for i in range(len(stage_out_channel)):
        if i == 0 :
            overall_channel += [math.ceil(stage_out_channel[i] * (stage_oup_cprate[i]))]
        else:
            overall_channel += [math.ceil(stage_out_channel[i] * (stage_oup_cprate[i]))]
            mid_channel += [math.ceil(stage_out_channel[i] * (mid_cprate[i-1]))]
...
def resnet20_fp(compress_rate=[1.0]*12):
    return ResNet(BasicBlock, 20, compress_rate=compress_rate)

def resnet32_fp(compress_rate=[1.0]*18):
    return ResNet(BasicBlock, 32, compress_rate=compress_rate)

def resnet44_fp(compress_rate=[1.0]*24):
    return ResNet(BasicBlock, 44, compress_rate=compress_rate)

def resnet56_fp(compress_rate=[1.0]*30):
    return ResNet(BasicBlock, 56, compress_rate=compress_rate)

def resnet110_fp(compress_rate=[1.0]*57):
