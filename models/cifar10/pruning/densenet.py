...
class DenseNet(nn.Module):
    ...
    def _make_denseblock(self, block, blocks, compress_rate):
        ...
        for i in range(blocks):
            layers.append(block(self.inplanes, outplanes=math.ceil(self.growthRate*(compress_rate[i])), dropRate=self.dropRate))
            self.inplanes += math.ceil(self.growthRate*(compress_rate[i]))

        return nn.Sequential(*layers)

    def _make_transition(self, transition, compressionRate, compress_rate):
        inplanes = self.inplanes
        outplanes = math.ceil(self.inplanes*(compress_rate) // compressionRate)
...
def densenet40_fp(compress_rate=[1.0]*39):
