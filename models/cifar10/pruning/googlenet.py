    ...
    def __init__(self, in_planes, n1x1, n3x3red, n3x3, n5x5red, n5x5, pool_planes,tmp_name, cp_rate, last=False):
        ...
        if self.n5x5 > 0:
            ...
            if last:
                ...
                output=math.ceil(n5x5*cp_rate)

            conv5x5_1 = nn.Conv2d(in_planes, n5x5red, kernel_size=1)
            conv5x5_2 = nn.Conv2d(n5x5red, math.ceil(n5x5*cp_rate), kernel_size=3, padding=1)
            conv5x5_3 = nn.Conv2d(math.ceil(n5x5*cp_rate), output, kernel_size=3, padding=1)
            ...
            self.branch5x5 = nn.Sequential(
                ...
                nn.ReLU(True),
                conv5x5_2,
                nn.BatchNorm2d(math.ceil(n5x5*cp_rate)),
                nn.ReLU(True),
                conv5x5_3,
                nn.BatchNorm2d(output),
                nn.ReLU(True),
...
class GoogLeNet(nn.Module):
    def __init__(self, compress_rate, block=Inception, filters=None):
        ...
        for cp_rate in compress_rate:
            cp_rate_list.append(cp_rate)

        in_plane_list=[]
        for i in range(8):
            in_plane_list.append(filters[i][0]+math.ceil(filters[i][1]*cp_rate_list[i+1])+math.ceil(filters[i][2]*cp_rate_list[i+1])+filters[i][3])
...
def googlenet_fp(compress_rate=[1.0]*10):
