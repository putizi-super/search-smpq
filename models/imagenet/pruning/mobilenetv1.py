...
class MobileNetV1(nn.Module):
    def __init__(self, compress_rate, num_classes=1000):
        ...
        def conv_dw(inp, oup, stride, inp_cprate, oup_cprate):
            return nn.Sequential(
                nn.Conv2d(math.ceil(inp*(inp_cprate)), math.ceil(inp*(inp_cprate)), 3, stride, 1, groups=math.ceil(inp*(inp_cprate)), bias=False),
                nn.BatchNorm2d(math.ceil(inp*(inp_cprate))),
                nn.ReLU(inplace=True),

                nn.Conv2d(math.ceil(inp*(inp_cprate)), math.ceil(oup*(oup_cprate)), 1, 1, 0, bias=False),
                nn.BatchNorm2d(math.ceil(oup*(oup_cprate))),
                nn.ReLU(inplace=True),
            )
...
def mobilenetv1_fp(compress_rate=[1.0]*13):
