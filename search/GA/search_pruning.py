...
def random_can(args, num, num_states, flops, params, total_flops, total_params):
    ...
    while(len(candidates))<num:
        if args.max_FLOPs == 1 and args.max_PARAMs == 1:
            can = np.ones(num_states+2)
            can[-1] = total_flops
            can[-2] = total_params            
        else:
            # # uniform random init
            ...
            if args.max_FLOPs == 1 and args.max_PARAMs !=1:
                if total_params/sparse_total_params < args.max_PARAMs or total_params/sparse_total_params > 1.1*args.max_PARAMs:
                    continue
            elif args.max_FLOPs != 1 and args.max_PARAMs ==1:
                if total_flops/sparse_total_flops < args.max_FLOPs or total_flops/sparse_total_flops >  1.1*args.max_FLOPs:
                    continue
            else:
                if total_flops/sparse_total_flops < args.max_FLOPs or total_flops/sparse_total_flops >  1.1*args.max_FLOPs or total_params/sparse_total_params < args.max_PARAMs or total_params/sparse_total_params > 1.1*args.max_PARAMs:
                    continue
            ...
            # compare difference
            t_can_int = [round(i*100.0) for i in t_can]
...
def get_mutation(args, epoch, keep_top_k, top_k_acc,num_states, mutation_num, m_prob, strength, flops,params,total_flops,total_params):
    ...
    for g_can in keep_top_k:
        global_candidates_int.append([round(i*100.0) for i in g_can[:-2]])
    k = len(keep_top_k)
    iter = 0
...
def get_crossover(args,keep_top_k, top_k_acc, num_states, crossover_num,flops,params,total_flops,total_params):
