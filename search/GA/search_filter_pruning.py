...
def random_can_fp(args, input_size, num, num_states, total_flops, total_params):
    ...
    while(len(candidates))<num:
        if args.max_FLOPs_FP == 1 and args.max_PARAMs_FP == 1:
            ...
            mask1 = can > 1.0
            mask2 = can <= 0.01
            can = mask1 * 1.0 + (1 - mask1) * can
            can = mask2 * 0.01 + (1 - mask2) * can

            t_can = tuple(can[:-2])
            # print(can[:-2].tolist())
            model = models.__dict__[args.arch](can[:-2].tolist()).to(device)

            layer_flops = calc_model_flops(model,input_size,mul_add=False)
            ...
            if args.max_FLOPs_FP == 1 and args.max_PARAMs_FP !=1:
                if total_params/sparse_total_params < args.max_PARAMs_FP or total_params/sparse_total_params > 1.1*args.max_PARAMs_FP:
                    continue
            elif args.max_FLOPs_FP != 1 and args.max_PARAMs_FP ==1:
                if total_flops/sparse_total_flops < args.max_FLOPs_FP or total_flops/sparse_total_flops >  1.1*args.max_FLOPs_FP:
                    continue
            else:
                if total_flops/sparse_total_flops < args.max_FLOPs_FP or total_flops/sparse_total_flops >  1.1*args.max_FLOPs_FP or total_params/sparse_total_params < args.max_PARAMs_FP or total_params/sparse_total_params > 1.1*args.max_PARAMs_FP:
                    continue
