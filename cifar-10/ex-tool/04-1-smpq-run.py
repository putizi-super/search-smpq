...
def decode_log(txt):
    ...
    rg1 = re.compile(re1+re2+re3+re4+re5+re6+re7+re8+re9+re10+\
        re11+re12+re13+re14+re15+re16+re17+re18+re19+re20+re21+re22,re.IGNORECASE|re.DOTALL)
    ...
    if m:
        ...
        search_loss=float(m.group(10))
        fineturn_acc=float(m.group(11))
...
def main():
    ...
    for f_bw in f_bws:
        for w_bw in w_bws:
            os.system('sh 04-0-smpq-ex.sh'+ ' ' +str(model)+ ' ' + str(w_bw)+ ' ' + str(f_bw) + ' ' + str(gpu_id))

            result_sheet = workbook.add_sheet('Result-'+'-f-1-p-1-w_bw-'+str(w_bw)+'-f_bw-'+str(f_bw))
            style = xlwt.XFStyle()
            alignment = xlwt.Alignment()
            alignment.horz = 0x02
            alignment.vert = 0x01
            style.alignment = alignment
            LAB_REPORT=['FLOPS','Pruning-Flops','Params','Pruning-Params','Avg-BW-Weights','Pruning-Bw','Avg-BW-Activation','Pruning-Fw','search-Acc','search-Loss','Fineturn-Acc']
            for i in range(len(LAB_REPORT)):
                result_sheet.write(0,i, LAB_REPORT[i], style)
            fname = '../experiment/cifar10/'+str(model)+'/search_fineturn/search_fineturn-f-1-p-1-w_bw-'+str(w_bw)+'-f_bw-'+str(f_bw)+'/'+str(model)+'-search_fineturn-f-1-p-1-w_bw-'+str(w_bw)+'-f_bw-'+str(f_bw)+'.log'
            ...
            workbook.save('../experiment/cifar10/'+str(model)+'/search_fineturn/search_fineturn-f-1-p-1-w_bw-'+str(w_bw)+'-f_bw-'+str(f_bw)+'/result.xls')
