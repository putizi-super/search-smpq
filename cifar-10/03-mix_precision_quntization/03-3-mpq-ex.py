...
def search_test(model, testLoader, topk=(1,)):
...
def test_candidates_model(epoch, candidates_bw_weights, candidates_bw_fm, candidates_prune):
    ...
    for can_bw_weights, can_bw_fm, can_prune in zip(candidates_bw_weights, candidates_bw_fm, candidates_prune):
        ...
        top1 = search_test(model_sq, search_loader.vaildLoader, topk=(1, 5) if args.data_set == 'imagenet' else (1, ))
        del(model_sq)
        del(model) 
...
def main():
    ...
    if args.baseline == True:
        ...
        model = init_params(model_baseline,model,w_bit,loader.trainLoader)
        model, avg_loss = Adaptive_BN(model,loader.trainLoader)
    else:
        ## scrach ->  Adaptive-BN
        model,avg_loss = Adaptive_BN(model,loader.trainLoader)

    test(model, loader.testLoader, topk=(1, 5) if args.data_set == 'imagenet' else (1, ))
    ...
    print('Sparse FLOPs = {:.2f} M | {:.2f} X | Sparse Prams = {:.2f} M (num) | {:.2f} X | AvgBit(w) = {:.2f} bit | {:.2f} X | AvgBit(FeatureMap) = {:.2f} bit | {:.2f} X | search_acc = {:.3f}% | fineturn_acc = {:.3f}%'\
        .format(sparse_flops/1000000,compression_sparse_flops,sparse_params/1000000,compression_sparse_params,32/compression_quantization_params,compression_quantization_params,32/compression_featuremap,compression_featuremap,candidates_acc[0],float(best_acc)))
