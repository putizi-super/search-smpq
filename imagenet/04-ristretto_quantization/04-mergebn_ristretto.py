...
# wbit=[32.0, 8.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0]
# wbit=[8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 32.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0,8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0,8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0]
# abit=[8.0, 32.0, 8.0, 8.0, 8.0, 8.0, 8.0, 8.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0]
# abit=[8.0]*len(wbit)
...
# darknet19 channel_keep  /loss 2.7641 X6
# w_bit=[32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0]
# a_bit=[32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0, 32.0]
...
def find_alpha_value_per_layer(x,bw):
...
def ristretto_init(model_baseline, model, trainLoader, epoch):
    model_baseline.eval()
    model.eval()
    cnt = 0
    alpha_weight = []
    ...
    for m in model_baseline.modules():
        if (isinstance(m, nn.Conv2d)) or (isinstance(m, nn.Linear)):
            ...
            alpha_weight.append(find_alpha_value_per_layer(weight,args.avg_bw_weights))
            bias = copy.deepcopy(m.bias.data)
            biases.append(bias)
            alpha_bias.append(find_alpha_value_per_layer(bias,args.avg_bw_weights))
            cnt += 1

    alpha_weight = Parameter(torch.Tensor(alpha_weight).to(device))
    alpha_bias = Parameter(torch.Tensor(alpha_bias).to(device))
    alpha_activation = Parameter(5*torch.ones(cnt).to(device))
    ...
    optimizer_mes_bias = torch.optim.SGD([alpha_bias], lr=0.1, momentum=0.9, weight_decay=4e-5)  
    optimizer_mes_act = torch.optim.SGD([alpha_activation], lr=0.1, momentum=0.9, weight_decay=4e-5)  
    # scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer_mes, epoch)
    scheduler_weights = optim.lr_scheduler.ReduceLROnPlateau(optimizer_mes_weights, min_lr=1e-5, verbose=False, patience=50)
    scheduler_bias = optim.lr_scheduler.ReduceLROnPlateau(optimizer_mes_bias, min_lr=1e-5, verbose=False, patience=50)
    scheduler_act = optim.lr_scheduler.ReduceLROnPlateau(optimizer_mes_act, mode='min', factor=0.1, min_lr=1e-5, verbose=True, patience=50)
    for i in range(epoch):
        ...
        for batch, batch_data in enumerate(trainLoader):
            if batch < 200:
                # Adeptive bias Step Size
                optimizer_mes_bias.zero_grad()
                bias_loss = 0
                for i in range(cnt):
                    bias = biases[i]
                    bias_f = bias.to(device)
                    scale_b = (2.0 ** round_pass(torch.log2(alpha_bias[i])) / (2.0 ** (args.avg_bw_weights-1)))
                    bias = bias / scale_b
                    bias = torch.clamp(bias, - 2 ** (args.avg_bw_weights - 1), 2 ** (args.avg_bw_weights - 1) -1 )
                    bias = round_pass(bias)
                    bias_q = (bias * scale_b).to(device)
                    bias_loss += loss_mse(bias_f,bias_q)

                print('bias Loss = ' + str(bias_loss.data)) 
                bias_loss.backward()
                optimizer_mes_bias.step()
                scheduler_bias.step(bias_loss.item())
        ...
        for m in model.modules():
            if (isinstance(m, nn.Conv2d)) or (isinstance(m, nn.Linear)):
                ...
                m.b_alpha.data = alpha_bias[cnt].data.to(device)
                cnt = cnt + 1

        # Adeptive Activation Step Size
        for batch, batch_data in enumerate(trainLoader):
            if batch < 300:
    ...
    print((torch.round(torch.log2(alpha_bias))-7))
    print((torch.round(torch.log2(alpha_activation))-7))
    return alpha_weight, alpha_bias, alpha_activation
...
def train(model, optimizer, trainLoader, args, epoch, topk=(1,)):
    ...
    print_freq = trainLoader._size // 256 // 10
    # keep_channel_freq = trainLoader._size // args.train_batch_size // 200
    start_time = time.time()
    for batch, batch_data in enumerate(trainLoader):
        inputs = batch_data[0]['data'].to(device)
        targets = batch_data[0]['label'].squeeze().long().to(device)

        lr = adjust_learning_rate(optimizer, epoch, batch, trainLoader._size // 256)
        ...
        if batch % print_freq == 0 and batch != 0:
            ...
            logger.info(
                ...
                'Top5 {:.2f}%\t'
                'Time {:.2f}s'.format(
                    epoch, batch * 256, trainLoader._size,
...
def init_params(model, trainLoader):
    ...
    optimizer = optim.Adam(filter(lambda p: p.requires_grad == True, model.parameters()), lr=1e-6, weight_decay=1e-5)
    # optimizer = optim.SGD(filter(lambda p: p.requires_grad == True, model.parameters()), lr=0.01)
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, min_lr=1e-7, verbose=False, patience=200)
...
def main():
    ...
    params = get_params_mergebn(model_baseline)
    model_mergebn = fillbias(model).to(device)
    model_mergebn = convert_to_mergebn(model_mergebn).to(device)

    model_baseline_mergebn = utils.load_params_model(copy.deepcopy(model_mergebn),params)
    model = convert_to_ristretto(copy.deepcopy(model_mergebn), [8]*len(w_bit), [32]*len(a_bit), pruning_rate).to(device)

    # print(model)
    if args.baseline == True:
        model = utils.load_params_model(model,params)

    alpha_weight, alpha_bias, alpha_activation = ristretto_init(model_baseline_mergebn, model, trainLoader, 1)
    ...
    ristretto_cnt[0] = 0
    model = convert_to_ristretto(copy.deepcopy(model_mergebn), [8]*len(w_bit), [8]*len(a_bit), pruning_rate).to(device)
    ...
    if len(args.gpus) != 1:
        model = BalancedDataParallel(int(256//len(args.gpus)//2), model, dim=args.gpus[0], device_ids=args.gpus)
        cudnn.benchmark = True
    
    # model = init_params(model, trainLoader) #fintuning
    ristretto_top1_acc, ristretto_top5_acc = test(model, testLoader, topk=(1, 5) if args.data_set == 'imagenet' else (1, ))
    ...
    state = {
        ...
        'best_top1_acc': ristretto_top1_acc,
        'best_top5_acc': ristretto_top5_acc
    }
