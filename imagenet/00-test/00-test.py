...
def filter_prune(model, example_inputs, output_transform):
    model.cpu().eval()
    prunable_module_type = ( nn.Conv2d, nn.BatchNorm2d, LambdaLayer)
...
def main():
    ...
    fp_model = filter_prune(model, torch.randn(1, 3, 224, 224), output_transform=None)

    featuremap = calc_model_featuremap(fp_model, input_size)
